...
import com.demussong.streaming.Sink.ChainedReference;
 ...
 class ReferenceChain<OUT, R> {
    ...
    public ReferenceChain<OUT, R> filter(Predicate<OUT> predicate) {
        ...
        sinkBuilders.add(() -> {
            ...
            Sink.ChainedReference<OUT, OUT> currentSink = (ChainedReference<OUT, OUT>) new ChainedReference<Object, OUT>(prevSink) {

                @Override
                public void accept(Object out) {
                    if (predicate.test((OUT) out)) {
    ...
    public ReferenceChain<OUT, R> map(Function<OUT, R> function) {
        ...
        sinkBuilders.add(() -> {
            ...
            Sink.ChainedReference<OUT, OUT> currentSink = (ChainedReference<OUT, OUT>) new ChainedReference<Object, OUT>(
                    (Sink<OUT>) prevSink) {

                @Override
                public void accept(Object in) {
                    downstream.accept((OUT) function.apply((OUT) in));
                }
            };
            sinkReference.set(currentSink);
            return currentSink;
    ...
    public void forEachPrint(Collection<OUT> collection, boolean reverse) {
        ...
        if (reverse) {
            ...
            for (int i = sinkBuilders.size() - 1; i >= 0; i--) {
                Supplier<Sink<?>> supplier = sinkBuilders.get(i);
                // 每次调用会把后面的sink作为前一个sink的downstream
                stage = (Sink<OUT>) supplier.get();
            }
        }
